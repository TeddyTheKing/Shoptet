...
(async () => {
    ...
    try {
        const selector = '[data-filter-code="stock"]';
    ...
    if (!currentUrl.includes('?stock=1')) { // Check if the query parameter is present
        console.error(`Test failed: Expected query parameter "stock=1" not found in URL. Current URL: ${currentUrl}`);
        process.exit(1); // Exit with an error code if validation fails
    } else {
        console.log('Test passed: Query parameter "stock=1" updated correctly');
    }
